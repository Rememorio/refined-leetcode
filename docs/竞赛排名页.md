...
预测数据来自 [EntrantHub](https://entranthub.com/contests/leetcode)。扩展会按当前排名页的用户查询已生成的预测分；比赛刚结束时数据可能需要短暂等待。由于其 API 使用 Cloudflare 防护，扩展会短暂创建一个非激活标签页完成同源取数，加载完成后自动关闭。
