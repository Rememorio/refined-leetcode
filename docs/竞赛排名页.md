...
预测数据来自 [EntrantHub](https://entranthub.com/contests/leetcode)。扩展会按当前排名页的用户查询已生成的预测分；比赛刚结束时数据可能需要短暂等待。由于其 API 使用 Cloudflare 防护，扩展会在最小化的后台窗口中完成同源取数，并串行合并同一比赛的重复请求，不会在当前 Chrome 窗口中反复创建标签页。
