...
- 2022-02-24 完成初始版本
- 2022-11-27 更新预测数据源
- 2026-07-12 迁移到 EntrantHub

此功能源码 [src/content/pages/ranking](../src/content/pages/ranking)

预测数据来自 [EntrantHub](https://entranthub.com/contests/leetcode)。扩展会按当前排名页的用户查询已生成的预测分；比赛刚结束时数据可能需要短暂等待。由于其 API 使用 Cloudflare 防护，扩展会通过不可见的 Offscreen Document 完成同源取数，并串行合并同一比赛的重复请求，不会创建任何标签页或浏览器窗口。
